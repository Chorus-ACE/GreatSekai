...
let fontManagerSampleText: [SekaiLocale: String] = [.jp: "あなたの輝きが道を照らす", .en: "Your Spark Will Light the Way", .tw: "你的光芒照耀漫漫長路", .cn: "你的光芒会照亮前行之路", .kr: "당신의 반짝임이 길을 밝힌다"]
let storyViewerDefaultFont: [SekaiLocale: String] = [.jp: ".AppleSystemUIFont", .en: ".AppleSystemUIFont", .tw: ".AppleSystemUIFont", .cn: ".AppleSystemUIFont", .kr: ".AppleSystemUIFont"]
...
struct SettingsFontsView: View {
    ...
    @State var storyViewerFonts: [SekaiLocale: String] = [:]
...
struct SettingsFontsAdd: View {
    ...
    var body: some View {
        NavigationStack {
                    ...
                    if newFontSourceType == 0 {
                        HStack {
                            ...
                            TextField("", text: $newFontOnlineURL, prompt: Text(verbatim: "example.com/font.ttf"))
                                ...
                                .labelsHidden()
                                .wrapIf(platform == .iOS, in: { content in
                                    #if os(iOS)
            ...
            .formStyle(.grouped)
            .toolbar {
                if platform == .macOS || !newFontIsAdding {
                    ...
                    Section(content: {
                        ...
                        .emptyReplacement {
                            Text(fontManagerSampleText[.en]!)
                                .font(.custom(fontName, size: 18))
                                .fontWeight(fontManagerSampleTextFontWeight)
                                .typesettingLanguage(SekaiLocale.en.nsLocale.language)
...
//                        }
                    ...
                    Section {
                        if platform == .iOS && !FontManager.builtInFonts.contains(fontName) {
                            Button(role: .destructive, action: {
                ...
                .toolbar {
                    ...
                    if platform == .macOS && !FontManager.builtInFonts.contains(fontName) {
                        ToolbarItem(placement: .destructiveAction) {
                            Button(role: .destructive, action: {
                                if fontManager.getFontCahce(forFontName: fontName)?.fileName != nil { // Is not system font
                                    fontManager.removeFont(fontName: fontName)
...
final class FontManager: ObservableObject {
    ...
    func fontSupportsLocale(_ fontName: String, locale: SekaiLocale) -> Bool {
